...
class Authorization{
    ...
    {
        $loginUrl = viser_route_link('user.login');
    ...
    function authFailed($username)
    {
        ...
        if ($referrer == viser_route_link('user.login') ) $referrer = $referrer . '?redirect_to=' . home_url('/'.get_option('viser_user_panel_prefix')); // in rare case where user access /login/ page directly
    ...
    function verifyUser($userdata)
    {
        ...
        if(!viser_verify_captcha()){
            viser_redirect(home_url('login?captcha=failed') );
        }

        if ($verify != '') {
            viser_redirect( home_url('login?email=unverified') );
        }
        return $userdata;
    }
    ...
    function userProfile($user)
    {
        if ('' != $verify_email = get_the_author_meta('_Verify Email', $user->ID)) {
            ...
            <table class="form-table">
                <tr>
                    <th><label for="emailverify"><?php esc_html_e('Email Verification Link', VISERLAB_PLUGIN_NAME);?></label></th>
                    <td>
                        <?php $verification_link .= sprintf('%s?email=verify&login=%s&key=%s', viser_route_link('user.login'), rawurlencode($user->user_login), $verify_email); ?>
                        <input type="text" name="_Verify_Email" id="_Verify_Email" value="<?php echo esc_url($verification_link); ?>" class="regular-text" /><br />
                        <span class="description"><?php esc_html_e('Leave blank to allow user to login without email verification.', VISERLAB_PLUGIN_NAME);?></span>
                    </td>
                </tr>
            </table>
        <?php
