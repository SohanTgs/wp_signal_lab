...
use Viserlab\BackOffice\MiddlewareHandler;
...
class ExecuteRouter{
    ...
    public function execute()
    {
        ...
        foreach ($routes as $routeKey => $routeValue) {
            if (array_key_exists('uri',reset($routeValue))) {
                ...
                if (empty($params)) {
                    add_rewrite_rule($route['uri'], 'index.php?viser_page='.$route['uri'], 'top');
                }else{
                    add_rewrite_rule($exactUri.'$', 'index.php?viser_page='.str_replace('//','/',$uri).$paramMatch, 'top' );
                }
            }
        }
        flush_rewrite_rules();
    ...
    {
        if (isset(viser_request()->page) && isset(viser_request()->module)) {
            if (viser_request()->page == VISERLAB_PLUGIN_NAME) {
                $action = viser_request()->module;
                $routes = Router::$routes;
                foreach ($routes as $routeKey => $routeValue) {
