...
class Captcha{
    ...
    */
    public static function verifyGoogleCaptcha(){
        ...
        if ($googleCaptcha) {
            $resp = json_decode(file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=".json_decode($googleCaptcha->shortcode)->secret_key->value."&response=".viser_request()->{'g-recaptcha-response'}."&remoteip=".viser_real_ip()), true);
            if (!$resp['success']) {
                $pass = false;
            }
        }
    ...
    */
    public static function verifyCustomCaptcha(){
        ...
        if ($customCaptcha) {
            $captchaSecret = hash_hmac('sha256', viser_request()->captcha, json_decode($customCaptcha->shortcode)->random_key->value);
            if ($captchaSecret != viser_request()->captcha_secret) {
                $pass = false;
            }
        }
        return $pass;
