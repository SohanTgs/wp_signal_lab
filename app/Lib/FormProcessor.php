        ...
        if ($forms) {
            for ($i=0; $i < count($forms['form_label']); $i++) {
                ...
                if ($extensions != 'null' && $extensions != null) {
                    ...
                    if ($notMatchedExt > 0) {
                        throw new \Exception( __("Your selected extensions are invalid", VISERLAB_PLUGIN_NAME) );
                    }
                }
                $label = viser_title_to_key(sanitize_text_field($forms['form_label'][$i]));
                $formData[$label] = [
        ...
        foreach($formData as $data){
            ...
            if($data->type == 'file') {
                if($request->hasFile($name)){
                    ...
                    $path = viser_file_path('verify').'/'.$directory;
                    $value = $directory.'/'.viser_file_uploader($value, $path);
                }else{
                    $value = null;
                }
            }
