...
namespace Viserlab\BackOffice\Validator;
...
class Validator{
    ...
    private static function requireValidation($mainRule){
        ...
        if ($mainRule == 'required' && !$hasValue && self::$inputValue != 0) {
            ...
            self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is required');
        }
    }
    ...
    private static function minValidation($mainRule,$ruleParam){
        if ($mainRule == 'min') {
            ...
            if (strlen(self::$inputValue) < $ruleParam) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be greater than '.$ruleParam.' characters');
            }
        }
    }
    ...
    private static function maxValidation($mainRule,$ruleParam){
        if ($mainRule == 'max') {
            ...
            if (strlen(self::$inputValue) > $ruleParam) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be less than '.$ruleParam.' characters');
            }
        }
    }
    ...
    private static function emailValidation($mainRule){
        if ($mainRule == 'email') {
            if (!sanitize_email(self::$inputValue)) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be a valid email');
            }
        }
    }
    ...
    private static function passwordConfirmValidation($mainRule){
        if ($mainRule == 'confirmed') {
            ...
            $confirmation = @viser_request()->password_confirmation;
            $validateRuleKey = self::$fieldName.'.'.$mainRule;
            if (!$confirmation) {
    ...
    private static function integerValidation($mainRule){
        if ($mainRule == 'integer') {
            if (!filter_var(self::$inputValue, FILTER_VALIDATE_INT)) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be an integer value');
            }
        }
    }
    ...
    private static function numericValidation($mainRule){
        if ($mainRule == 'numeric') {
            if (!is_numeric(self::$inputValue)) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be a numeric value');
            }
        }
    }
    ...
    private static function gtValidation($mainRule,$ruleParam){
        if ($mainRule == 'gt') {
            ...
            if ($ruleParam >= intval(self::$inputValue)) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be grater than '.$ruleParam);
            }
        }
    }
    ...
    private static function gteValidation($mainRule,$ruleParam){
        if ($mainRule == 'gte') {
            ...
            if ($ruleParam > intval(self::$inputValue)) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be grater than or equal '.$ruleParam);
            }
        }
    }
    ...
    private static function ltValidation($mainRule,$ruleParam){
        if ($mainRule == 'lt') {
            ...
            if ($ruleParam <= intval(self::$inputValue)) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be less than '.$ruleParam);
            }
        }
    }
    ...
    private static function lteValidation($mainRule,$ruleParam){
        if ($mainRule == 'lte') {
            ...
            if ($ruleParam < intval(self::$inputValue)) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be less than or equal '.$ruleParam);
            }
        }
    }
    ...
    private static function fileValidation($mainRule){
        if ($mainRule == 'file') {
            if (!is_file(is_array(self::$inputValue) ? self::$inputValue['tmp_name'] : '')) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be a valid file');
            }
        }
    }
    ...
    private static function imageValidation($mainRule){
        if ($mainRule == 'image' && self::$inputValue['tmp_name']) {
            ...
            if (!in_array($image_type , array(IMAGETYPE_GIF , IMAGETYPE_JPEG ,IMAGETYPE_PNG , IMAGETYPE_BMP))) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field is must be a valid image');
            }
        }
    }
    ...
    private static function mimesValidation($mainRule,$ruleParam){
        if ($mainRule == 'mimes' && self::$inputValue['tmp_name']) {
            ...
            if (!in_array($ext,explode(',',$ruleParam))) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' file\'s extension is not a valid extension');
            }
        }
    }
    ...
    private static function sizeValidation($mainRule,$ruleParam){
        if ($mainRule == 'size' && self::$inputValue['tmp_name']) {
            ...
            if ($size > $ruleParam || !$size) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' File size could not be greater than '.$ruleParam.' KiloBite');
            }
        }
    }
    ...
    private static function inValidation($mainRule,$ruleParam){
        if ($mainRule == 'in') {
            ...
            if (!in_array(self::$inputValue,explode(',',$ruleParam))) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' filed value is not valid');
            }
        }
    }
    ...
    private static function requiredIfValidation($mainRule,$ruleParam){
        if ($mainRule == 'required_if') {
            ...
            $requestValue = @viser_request()->$ruleParam;
            $hasValue = false;
            if ($requestValue) {
                $hasValue = true;
            }
            ...
            if ($hasValue && !self::$inputValue) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' filed is required when '.viser_key_to_title($ruleParam).' is exists');
            }
        }
    }
    ...
    private static function regexValidation($mainRule,$ruleParam){
        if ($mainRule == 'regex') {
            ...
            if (!preg_match($ruleParam,self::$inputValue)) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field value is not valid');
            }
        }
    }
    ...
    private static function urlValidation($mainRule){
        if ($mainRule == 'url') {
            if (!wp_http_validate_url(self::$inputValue)) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field value is not a valid URL');
            }
        }
    }
    ...
    private static function uniqueValidation($mainRule,$ruleParam){
        if ($mainRule == 'unique') {
            ...
            if ($exists) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field value must be unique');
            }
        }
    }
    ...
    private static function existsValidation($mainRule,$ruleParam)
    {
        if ($mainRule == 'exists') {
            ...
            if (!$exists) {
                $validateRuleKey = self::$fieldName.'.'.$mainRule;
                self::$validationRules[$validateRuleKey] = self::getMessage($validateRuleKey,viser_key_to_title(self::$fieldName).' field value not exists');
            }
        }
    }
